...
- **Закрепленные посты**
  - Запрос закреплённого поста профиля и показ первым в ленте профиля
  - Визуальное выделение (pin рядом с именем + акцентная обводка)

- **Управление кешем**
  - Исправлены отступы/верстка в разделе управления кешем (заголовки и диаграмма)

- **Посты / модалки**
  - Исправлена подгрузка данных в модалке поста (last_comment, likes_count, is_liked)
  - Кнопка «тут пусто» для last comment: в модалке без текста и некликабельна

- **Удаление**
  - Подтверждение перед удалением постов/комментариев/ответов
  - Исправлен запрос удаления поста: DELETE `/api/posts/{id}` (и фиксы для меню)

- **Редактирование постов**
  - Добавлено редактирование поста и запрос `POST /api/posts/{id}/edit` (multipart) + обновление поста в UI

- **Формат дат**
  - Приведено к `d MMM` и больше не зависит от системного 12/24‑часового формата

- **Текст постов**
  - Исправлено обрезание текста и логика «Показать полностью» для длинных постов

- **Просмотр фото**
  - Кнопка сохранения изображения в «Фото»
  - Плавное затемнение верхней safe‑area (часы/батарея) в полноэкранном просмотре
